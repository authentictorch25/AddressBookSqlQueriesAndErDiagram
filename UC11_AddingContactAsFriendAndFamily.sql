...
select * from AddressBook where city = 'Waidhan' order by first_name ;

Alter table AddressBook add bookName varchar(50), type varchar(30);

Update AddressBook set bookName = 'favourites' ;
Update AddressBook set type = 'friends' where first_name = 'Namrata' ;
Update AddressBook set type = 'family' where first_name ='Ashutosh' or first_name= 'Akash' ;
select * from AddressBook;

select type,COUNT(first_name) AS num_of_contact from AddressBook Group by type ;

Insert into AddressBook (first_name,last_name,address,city, state,zip, phone,email,bookName,type) 
values
('Namrata', 'Yadav', '41/728','Vindhyanagar', 'MadhyaPradesh',486885,9407387972,'namrata@gmail.com','favourites','family')

alter table AddressBook Add name varchar(100);
update AddressBook set name = first_name+' '+last_name;
